...
public class SupplierPortImpl implements SupplierPortType {
	...
	public List<ProductView> searchProducts(String descText) throws BadText_Exception {
		// TODO
		List<ProductView> lst = new ArrayList<ProductView>();
		Supplier supplier = Supplier.getInstance();
		
		if(descText == null || descText.length() < 1 || "                         ".contains(descText) || descText.equals("\n") || descText.equals("\t"))
			throwBadText("Text can't be empty, null, or just spaces.");
		for(String id : supplier.getProductsIDs())
			if(supplier.getProduct(id).getDescription().contains(descText) == true)
				lst.add(newProductView(supplier.getProduct(id)));
	...
	public String buyProduct(String productId, int quantity)
		...
		try{
			...
			throwBadQuantity("Bad quantity exception");
		}
		return null;
	}
